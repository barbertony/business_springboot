...
import com.neuedu.vo.CartOrderItemListVO;
...
public class OrderServiceImpl implements IOrderService {
    ...
    public ServerResponse createOrder(Integer userId, Integer shippingId) {
        ...
        cleanCart(cartList);
        orderItemList=orderItemMapper.findOrderItemByOrderNo(order.getOrderNo());
        OrderVO orderVO=assembledOrderVO(order,orderItemList,shippingId);
    ...
    public ServerResponse cancel(Integer userId, Long orderNo) {
        ...
        if (order.getStatus()!=Const.OrderStatusEnum.ORDER_UN_PAY.getCode())
        {
            ...
            {
                return ServerResponse.createServerResponseByFail(1,"此订单已付款，无法被取消");
            }
    ...
    public ServerResponse get_order_cart_product(Integer userId) {
        ...
        CartOrderItemListVO cartOrderItemListVO=new CartOrderItemListVO();
        cartOrderItemListVO.setImageHost("imageHost");
        List<OrderItem> orderItemList=(List<OrderItem>) serverResponse.getData();
        List<OrderItemVO> orderItemVOList=Lists.newArrayList();

        if (orderItemList==null||orderItemList.size()==0)
        {
            return ServerResponse.createServerResponseByFail(9,"购物车为空");
        }
        for (OrderItem orderItem:orderItemList)
        {
            OrderItemVO orderItemVO=assembledOrderItemVO(orderItem);
            orderItemVOList.add(orderItemVO);
        }

        cartOrderItemListVO.setOrderItemVOList(orderItemVOList);
        BigDecimal totalPrice=getOrderPrice(orderItemList);
        cartOrderItemListVO.setTotalPrice(totalPrice);



        return ServerResponse.createServerResponseBySuccess(null,cartOrderItemListVO);
    }
    ...
    public ServerResponse list(Integer userId, Integer pageSize, Integer pageNum) {
        ...
        {List<OrderItem> orderItemList=orderItemMapper.findOrderItemByOrderNo(order.getOrderNo());
            ...
            OrderVO orderVO= assembledOrderVO(order,orderItemList,order.getShippingId());
    ...
    public ServerResponse detail(Integer userId, Long orderNo) {
        ...
        if (userId==null)
        {
            order=orderMapper.findOrderByOrderNo(orderNo);
        }
        else
        {
            order=orderMapper.findOrderByUserIdAndOrderNo(userId,orderNo);
        }
        if (order==null)
        {
            return  ServerResponse.createServerResponseByFail(5,"未查询到订单信息");
    ...
    public ServerResponse search(Long orderNo,Integer pageNum,Integer pageSize) {
        ...
        String strNo = String.valueOf(orderNo);
        if (strNo.length()==13)
        {
        ...
        {
            return ServerResponse.createServerResponseByFail(1,"没有找到订单");
        }
        else
        {
            String strOrderMax=strNo;
            String strOrderMin=strNo;
            for (int i = strNo.length(); i <13 ; i++) {
                strOrderMax=strOrderMax+"9";
                strOrderMin=strOrderMin+"0";
            }
            Long orderMax=Long.valueOf(strOrderMax);
            Long orderMin=Long.valueOf(strOrderMin);
            orderList=orderMapper.searchOrderByOrderNo(orderMin,orderMax);
        }
    ...
    public ServerResponse send_goods(Long orderNo) {
        ...
        if (order.getStatus()==Const.OrderStatusEnum.ORDER_PAYED.getCode())
        {
            ...
            int result=orderMapper.send_goods(order);
            if (result<1)
            {
                return ServerResponse.createServerResponseByFail(1,"发货失败");
            }
            return ServerResponse.createServerResponseBySuccess(null,"发货成功");
        }
        return ServerResponse.createServerResponseByFail(1,"发货失败");
    }
    ...
    private OrderItemVO assembledOrderItemVO(OrderItem orderItem)
    {
        ...
        if (orderItem!=null)
        {
            ...
            orderItemVO.setCreateTime(dateFormat.format(orderItem.getCreateTime()));
    ...
    private OrderVO assembledOrderVO(Order order,List<OrderItem> orderItemList,Integer shippingId)
    {
        ...
        orderVO.setImageHost("缺少工具类");
        Shipping shipping=shippingMapper.selectByPrimaryKey(shippingId);
        if (shipping!=null)
        {
            orderVO.setShippingId(shippingId);
            ShippingVO shippingVO =new ShippingVO();
            shippingVO.assembledShipping(shipping);
        ...
        orderVO.setPaymentTypeDesc(Const.PaymentEnum.codeOf(order.getPaymentType()).getDesc());
    ...
    {
        cartMapper.bathDelete(cartList);
    }
    ...
    private  BigDecimal getOrderPrice(List<OrderItem> orderItemList)
    {
        ...
        for (OrderItem o:orderItemList) {
            BigDecimal a=o.getTotalPrice();
            bigDecimal=BigDecimalUtils.add(bigDecimal.doubleValue(),a.doubleValue());
...
///////////////////////////////////////////////////////////////////////////////
    ...
    public ServerResponse alipay_callback(Map<String,String> map) {
        ...
        if (gmt_payment!=null)
        {
            ...
            strtodate = formatter.parse(gmt_payment, pos);
        }
        ...
        if (order==null)
        {
            return  ServerResponse.createServerResponseByFail(1,orderNo+"订单不是本商城的订单");

        }
        if (order.getStatus()>=20)
        {
            return  ServerResponse.createServerResponseByFail(1,"支付宝重复回调订单");
        ...
        if (trade_status.equals(Const.TRADE_SUCCESS))
        {
            ...
            if (gmt_payment!=null)
            {
                order.setPaymentTime(strtodate);
            }
            int result= orderMapper.updateByPrimaryKey(order);
        }
        PayInfo payInfo=new PayInfo();
        payInfo.setOrderNo(orderNo);
        payInfo.setPayPlatform(Const.PaymentPayInfoEnum.ALIPAY.getCode());
        ...
        if (result<1)
        {
            return ServerResponse.createServerResponseByFail(1,"支付信息保存失败");

        }



        return ServerResponse.createServerResponseBySuccess("success");
    }
    ...
    public List<Order> closeOrder(String closeTime) {
        List<Order> orderList=orderMapper.selectOrderByCreateTime(closeTime);
        if (orderList!=null&orderList.size()>0)
        {
            for (Order order:orderList)
            {
    ...
    public ServerResponse pay(Order order) {
        ...
        String subject = "xxx品牌xxx门店当面付扫码消费"+"   订单"+order.getOrderNo()+"    金额"+order.getPayment().intValue();

        // (必填) 订单总金额，单位为元，不能超过1亿元
        // 如果同时传入了【打折金额】,【不可打折金额】,【订单总金额】三者,则必须满足如下条件:【订单总金额】=【打折金额】+【不可打折金额】
        System.out.println(String.valueOf(order.getPayment().doubleValue()));
        ...
        String body = "购买商品"+order.getPayment().intValue()+"元";
        ...
        AlipayTradePrecreateRequestBuilder builder = new AlipayTradePrecreateRequestBuilder()
                ...
                .setNotifyUrl(appConfig.getCall_back())//支付宝服务器主动通知商户服务器里指定的页面http路径,根据需要设置
                .setGoodsDetailList(goodsDetailList);

        AlipayF2FPrecreateResult result = tradeService.tradePrecreate(builder);
        System.out.println(appConfig.getCall_back());
        switch (result.getTradeStatus()) {
            case SUCCESS:
                log.info("支付宝预下单成功: )");
                ...
                // 需要修改为运行机器上的路径
                String filePath = String.format(appConfig.getQr_filepath(),
        ...
        return ServerResponse.createServerResponseByFail(1,"下单失败");
    }
